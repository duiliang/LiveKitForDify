...
class STTRequest(BaseModel):
    ...
    format: str = Field(
        "pcm", description="Audio container format supported by the Bailian API"
    )
    sample_rate: int = Field(
        16000,
        description="Sampling rate of the audio buffer. Bailian expects 16kHz by default",
    )
...
class TTSRequest(BaseModel):
    ...
    voice: Optional[str] = Field(
        None, description="Optional Bailian voice identifier for speech synthesis"
    )
    format: str = Field(
