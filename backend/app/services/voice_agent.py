...
from .dashscope import DashScopeError, synthesize_speech, transcribe_audio
...
async def run_agent(job_context: "JobContext", callbacks: Optional[AgentCallbacks] = None) -> None:
    ...
    2. Sends the captured buffers to DashScope real-time STT.
    3. Streams the transcript to Dify in order to obtain a response.
    4. Converts the reply to speech using DashScope CosyVoice TTS and publishes it back into
       the LiveKit room.
        ...
        async def on_track_subscribed(
            ...
            async def _process() -> None:
                ...
                try:
                    ...
                    logger.warning("DashScope STT failed: %s", exc)
                    return
                ...
                try:
                    ...
                    logger.warning("DashScope TTS failed: %s", exc)
